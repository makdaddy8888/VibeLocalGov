(function () {
  ...
  function buildPromptThoughts(ctx) {
    return [
      ...
      "",
      "This section explains how VibeLocalGov built your request. Share it with your team or keep it for records. The **only** text that belongs in the Vibe text box is **part 2** in the app (or the *Paste into Vibe* section in an export) — that part matches how Microsoft documents **natural language** input at vibe.powerapps.com.",
      "",
      "## How Power Apps Vibe works (per Microsoft Learn)",
      "",
      "Per Microsoft’s guidance, the Vibe experience is designed to turn a **plain-language description** of what you want to build into a working design path. Typical flow (see *Create apps, data, and plans together using Power Apps vibe* and *Overview of the new Power Apps vibe experience* on Microsoft Learn):",
      "",
      "1. Sign in at [https://vibe.powerapps.com](https://vibe.powerapps.com) (in a supported environment).",
      "2. Enter your prompt in the text box and submit — or use dictation, optional **Enhance prompt**, or suggestions, depending on what your tenant shows.",
      "3. Agents produce **assets together**: a **plan**, **data model** (tables/relationships), and an **app preview** in one workspace so you can iterate in chat before committing.",
      "4. **Save** stores the app, plan, and model into **your selected solution**; data may stay in **in-memory draft** form until you **publish** and **choose a data source** — so you can refine without locking in storage too early.",
      "5. **Review** all generated content before you save or share it; follow your **governance** (same idea as Microsoft’s Vibe FAQ: clear requirements, test in the right environment).",
      "",
      "Preview and rollout rules change by release. Your IT / Power Platform admin should confirm: Copilot, **non-default** environment, supported **region** and **language** — use Microsoft’s *Released versions - new Power Apps experience (preview)* and current release notes:",
      "https://learn.microsoft.com/en-us/power-platform/released-versions/new-powerapps",
      "",
      "Further reading: https://learn.microsoft.com/en-us/power-apps/vibe/overview and https://learn.microsoft.com/en-us/power-apps/vibe/create-app-data-plan",
      "",
      "## Microsoft Power Platform — why the details still matter for councils",
      "",
      "In Australian local government, good solutions keep **data in your tenant**, use **role-based access**, and keep **people in the loop** for any outcome that affects residents or staff. That does not change how you type the prompt; it shapes what you ask Vibe to **avoid** (PII) and what must stay **officer-approved**.",
      "",
      ...
      ...ctx.ideasLines,
      "",
      "## What part 2 is written to do (rationale)",
      "",
      "Microsoft’s instructions are to **describe your app idea in natural language** in the Vibe text box, then let agents return plan + data + app preview. The paste in part **2** therefore: (a) states the business scenario in a short opening paragraph, (b) lists roles and constraints, and (c) explicitly asks for the same kinds of **plan** / **data model** / **app** outputs Vibe is built to generate — so the model is not asked to do something outside the product’s flow.",
      "",
      "---",
      "Generated in-browser by VibeLocalGov — not an automated AI call.",
    ].join("\n");
  ...
  function buildPromptVibeOnly(ctx) {
    ...
    const who =
      (ctx.position || "Council officer") + " in " + ctx.workAreaText + (ctx.includeName && ctx.name ? " (" + ctx.name + ")" : "");
    return [
      "I am in Australian local government and need a Power Apps–based solution I can work on in Power Apps Vibe. Describe everything in process terms: do not use or invent real resident personal information; no addresses or PII. Officers must review anything customer-facing, enforcement-related, or legally binding. Work email context only: " + domain + ".",
      "",
      "Natural language ask (what to build):",
      "We need a solution that helps with the following day-to-day situation: " + ctx.pain + " The problem roughly costs " + (TIME_LABELS[ctx.timeCost] || ctx.timeCost) + " of our time per week. If we get it right, " + ctx.benefits,
      "",
      "Who and where:",
      "- Primary user story / role: " + who,
      "- Years in local government (context only): " + (TERM_LABELS[ctx.tenure] || ctx.tenure),
      "",
      "What I need Vibe to generate (per Microsoft’s plan + data + app flow):",
      "- A **plan** and **assumptions** I can read before changing anything.",
      "- A **data model** suggestion (entities/columns/relationships at a high level) that could live in **Microsoft Dataverse** in our environment when we are ready to publish; call out any integration points (e.g. CRM) at idea level only.",
      "- An **app preview** direction: main screens, navigation, and key user tasks that match the scenario above.",
      "- Explicit **in scope / out of scope** for a first iteration, and what needs **human sign-off** (service to the public, regulatory decisions, data exports, etc.).",
      "",
      "After the first pass I expect to use the Vibe **chat** to refine the plan, data, and app before **Save to a solution** and later **Publish** with a real data source, as described in Microsoft Learn for Vibe.",
    ].join("\n");
  }
